        ...
        <div className="selected-day-events">
            ...
            <Grid container spacing={2}>
                {value && value.docs.map(
                    (doc) => doc.data().time.toDate().getDate() == props.selectedDate.getDate() ? <Grid item xs={4}> <li className="calendar-events-items"><CalendarEvent title={doc.data().title} speakers={doc.data().speakers.join(',')} time={doc.data().time.toDate()} /></li> </Grid> : <></>
                )}
            </Grid>
        </div>
    )
